...
async function load() {
  loading.value = true
  try {
    const { data } = await api.get('/rekap-per-program', { params: { tahun: tahun.value } })
...
<template>
  <div>
    <div style="margin-bottom: 20px;">
      <h2 style="margin: 0; font-size: 18px; font-weight: 700; color: #303133;">Rekap Realisasi Per Program</h2>
      <p style="margin: 4px 0 0; font-size: 13px; color: #909399;">
        Anggaran vs Realisasi (SPP &amp; SP2D) per Program &rarr; SKPD &rarr; Unit SKPD &rarr; Urusan &rarr; Bidang Urusan &rarr; Kegiatan &rarr; Sub Kegiatan &rarr; Belanja
      </p>
    </div>
    ...
    <div style="display: flex; align-items: center; gap: 14px; margin-bottom: 16px; flex-wrap: wrap;">
      ...
      <span v-if="isFiltering" style="font-size: 12px; color: #909399;">
        {{ filteredNodes.length }} program ditampilkan
      </span>
    </div>
